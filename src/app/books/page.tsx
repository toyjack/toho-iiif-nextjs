...
export default function BooksPage() {
  ...
  const filterOptions = useMemo(() => {
    const categories = [...new Set(tohoData.books.map(book => book.category))].sort();
    const dynasties = [...new Set(tohoData.books.map(book => book.dynasty).filter(Boolean))].sort();
    const bookTypes = [...new Set(tohoData.books.map(book => book.bookType))].sort();
  ...
  const filteredAndSortedBooks = useMemo(() => {
    let filtered = tohoData.books.filter(book => {
      ...
      // 印章过滤
      if (filters.hasSeals !== null && book.hasSeals !== filters.hasSeals) return false;
      
      // 注释过滤
      if (filters.hasNotes !== null && book.hasNotes !== filters.hasNotes) return false;
      
      // 完整性过滤
      if (filters.isIncomplete !== null && book.isIncomplete !== filters.isIncomplete) return false;
      
      // 仅显示可浏览的书籍
      if (filters.onlyAvailable && !AvailableBook.includes(book.id)) return false;
  ...
  const paginatedBooks = filteredAndSortedBooks.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    ...
    <div className="min-h-screen bg-base-100">
      ...
      <div className="navbar bg-base-100 shadow-sm border-b border-base-200">
        ...
        <div className="navbar-center hidden lg:flex">
          <ul className="menu menu-horizontal px-1">
            <li><Link href="/" className="font-medium">首頁</Link></li>
            <li><Link href="/books" className="font-medium text-primary">古籍瀏覽</Link></li>
            <li><Link href="/viewer" className="font-medium">閱讀器</Link></li>
            <li><Link href="/about" className="font-medium">關於我們</Link></li>
          </ul>
        </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-base-content/70">探索 {tohoData.metadata.totalBooks} 部珍貴古籍典藏</p>
        </div>
                ...
                <div className="form-control mb-4">
                  ...
                  >
                    ...
                    {filterOptions.bookTypes.map(type => (
                      <option key={type} value={type}>{getBookTypeLabel(type)}</option>
                    ))}
                  </select>
                </div>
                  ...
                  <div className="form-control">
                    <label className="label cursor-pointer justify-start gap-3">
                      <input
                        ...
                        onChange={(e) => updateFilter('hasSeals', e.target.checked ? true : null)}
                      />
                  ...
                  <div className="form-control">
                    <label className="label cursor-pointer justify-start gap-3">
                      <input
                        ...
                        onChange={(e) => updateFilter('hasNotes', e.target.checked ? true : null)}
                      />
                  ...
                  <div className="form-control">
                    <label className="label cursor-pointer justify-start gap-3">
                      <input
                        ...
                        onChange={(e) => updateFilter('isIncomplete', e.target.checked ? false : null)}
                      />
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              <div className="flex items-center gap-2 text-sm text-base-content/70">
                <span>顯示 {startIndex + 1}-{Math.min(startIndex + ITEMS_PER_PAGE, filteredAndSortedBooks.length)} 項，</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
              {paginatedBooks.map((book) => {
                ...
                return (
                  <div key={book.id} className={`card bg-base-100 shadow-sm border transition-all duration-300 group ${
                    isAvailable 
                    ...
                    <div className="card-body p-6">
                      <div className="flex justify-between items-start mb-3">
                        <h3 className={`card-title text-lg line-clamp-2 ${
                          isAvailable ? 'group-hover:text-primary transition-colors' : 'text-base-content/70'
                        }`}>
                          {book.title}
                        </h3>
                        {isAvailable && (
                          <div className="badge badge-success badge-sm">可瀏覽</div>
                        )}
                      </div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {book.dynasty && (
                            <span className="badge badge-secondary badge-sm">{book.dynasty}</span>
                          )}
                          <span className="badge badge-outline badge-sm">{getBookTypeLabel(book.bookType)}</span>
                        </div>
